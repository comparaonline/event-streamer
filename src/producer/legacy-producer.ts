...
function warnDeprecation(message: string): void {
  try {
    ...
    // getConfig may throw if not initialized; skip in that case
  }
}
...
/**
 * @deprecated Use SchemaRegistryProducer.emitWithSchema() for new events and type safety.
 */
...
export async function emit(param1: string | Output | Output[], param2?: any, param3?: any): Promise<EmitResponse[]> {
  warnDeprecation('emit() is deprecated. Use SchemaRegistryProducer.emitWithSchema() for Schema Registry support.');
  const config = getConfig();
